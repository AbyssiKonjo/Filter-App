...
function attachModalToImages() {
    ...
    for (let i = 0; i < images.length; i++) {
        images[i].addEventListener('click', function(event) {
            ...
            const imageTop = rect.top + scrollPosition; // Calculate the top position for the clicked image
            const windowHeight = window.innerHeight;
            const dialogHeight = detailsModal.offsetHeight; // height of the modal
            const viewportTop = scrollPosition;

            let dialogTop = viewportTop + (windowHeight - dialogHeight) / 2; // Calculate the top position for dialog to be centered

            //Ensure the dialog doesnt go above or below the viewport:
            if (dialogTop < imageTop) {
                dialogTop = imageTop; // Place the dialog just below the image if theres enough space
